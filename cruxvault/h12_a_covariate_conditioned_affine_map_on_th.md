...
type: idea
schema: 2
title: A covariate-conditioned affine map on the log-count axis closes at least half of the gap that the covariate-free monotone map leaves, and beats its shuffled-covariate twin by more than the seed floor
parent: q4
status: idea
rule: all
...
# h12 — A covariate-conditioned affine map on the log-count axis closes at least half of the gap that the covariate-free monotone map leaves, and beats its shuffled-covariate twin by more than the seed floor

Parent:: [[q4_how_much_capacity_does_a_covariate_condi]]

## ELI5
Once both runs' settings are known, a stretch and a shift is enough to translate one run at least halfway to the other.

## TL;DR
The lowest rung of the capacity ladder: log mu_target = a · log mu_source + b, with a and b linear in the concatenated source and target covariate vectors, fitted on the counterfactual pairs of [[t112_build_the_counterfactual_arms_for_t|the counterfactual-arms task]]. The competitor — call it one-warp — is a single covariate-free monotone map on each output axis, fitted on training pairs pooled within this knob and scored on held-out pairs; the oracle is two independent NB draws from the same eta at the target depth (PI ruling 2026-09-16). Gap-closed is (D_onewarp − D_model) / (D_onewarp − D_oracle) with D the macro NB CRPS against the real target. The twin is the same affine map with the covariates shuffled across pairs. Supported if gap-closed is at least 0.5 on each output axis in every mark class, the gain over one-warp exceeds 2× the model's own seed |Δ| over the declared seeds, and the gain over the twin also exceeds 2× seed |Δ|; refuted if any of the three fails. Supported means the covariates act on the signal as a rescaling at this bar; it says nothing about what a larger model could add.
...
A linear-in-covariates affine map is the cheapest conditioned transformation; if it clears the bar the covariates act as a rescale and the higher rungs are not needed to answer q1.

## Idea / Hypothesis

A covariate-conditioned affine map on the log-count axis closes at least half of the gap that the covariate-free monotone map leaves, and beats its shuffled-covariate twin by more than the seed floor
