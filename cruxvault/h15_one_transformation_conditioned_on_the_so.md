...
status: idea
rule: all
measurement: "g(C, C') outputs f, and f(X) predicts X' as NB (n, p) per bin for counts and log-normal (μ, σ) per bin for −log10 p; g is what is trained, in two versions: one g per track (7 g's) and one g across all 7 tracks, both scored under the same checks (which version decides pass/fail: TODO(PI)); trained on base↔arm pairs of the t112 counterfactual corpus in both directions, validated on chr22, scored on chr19 + chr21 (blacklist removed) and on never-trained arm→arm pairs; D = NB CRPS / log-normal CRPS and Spearman, on all, non-zero and top-1% bins; competitor = the same model trained with C and C' scrambled across pairs; design record plan/T118_COUNTERFACTUAL_F.md"
...
- [ ] shufflecollapse: at scoring, C' replaced by the C' of another arm of the same track whose target differs (drawn separately for counts and p); g's advantage over the twin must vanish; bar: TODO(PI) — previously gap-closed ≤ 0.1 against the void oracle
      fails-if:: g does not use C': the f it outputs does not change when told the wrong target covariates
- [ ] swapreturn: at scoring, C' set equal to C; the f that g outputs must return X; bar: TODO(PI) — previously gap-closed toward X ≥ 0.5 against the void oracle
...
- g is trained, f is its output. Two versions of g: one per track (7 g's; the assay entry is constant inside each) and one across all 7 tracks. Both are scored under the same checks; which decides pass/fail is TODO(PI). No per-arm version.
