...
type: idea
schema: 2
title: Conditioning on the recorded antibody-efficiency proxy predicts a target track from a source track, beyond what a single value-axis map already does
parent: q2
status: idea
rule: all
measurement: gap-closed on the onewarp→oracle interval, per signal decile and per mark class, on both output axes, over the antibody-efficiency proxy counterfactual pair(s) against base; instrument is a standalone covariate-conditioned transformation testbed (encoder/decoder, or a lower-capacity conditioned map under q4) with log d pinned as a fixed offset of coefficient exactly 1, scored through bench.distributional.nb_suite and bench.covariate
...
# h6 — Conditioning on the recorded antibody-efficiency proxy predicts a target track from a source track, beyond what a single value-axis map already does

Parent:: [[q2_do_the_covariates_that_record_which_read]]

## ELI5
Knowing what fraction of a run's reads are really background closes at least half of the gap a plain rescale leaves.

## TL;DR
The pair is the base track and its counterfactual in which a fraction f = 0.5 or 0.9 of the treatment reads is replaced by reads drawn from the matched control with the pipeline's own subsampler, total reads fixed at 30M, f recorded as the covariate; the MACS2 control is unchanged ([[t112_build_the_counterfactual_arms_for_t|the counterfactual-arms task]]). The competitor — call it one-warp — is a single covariate-free monotone map on each output axis, fitted on training pairs pooled within this knob and scored on held-out pairs; the oracle is two independent NB draws from the same eta at the target depth (PI ruling 2026-09-16). Gap-closed is (D_onewarp − D_model) / (D_onewarp − D_oracle) with D the macro NB CRPS against the real target. Supported if gap-closed is at least 0.5 on both axes in every mark class and the gain over one-warp exceeds 2× the model's own seed |Δ|; refuted if either fails in any class. If the one-warp → oracle interval is itself below 2× seed |Δ|, the null is supported and gap-closed is not read. The same model with the covariate shuffled across pairs must collapse (a control), so a win belongs to the covariate and not to capacity.
...
A weaker antibody lowers signal-to-background without changing depth; the proxy mixes control reads in at a known fraction so the covariate is exact.

## Idea / Hypothesis

Conditioning on the recorded antibody-efficiency proxy predicts a target track from a source track, beyond what a single value-axis map already does

## Verifiables
<!-- Cloned 2026-09-17 from h1's PI-approved set with the knob = antibody-efficiency proxy; thresholds carried over, TODO(PI) where h1 says so. -->
