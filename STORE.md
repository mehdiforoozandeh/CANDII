  ...
  "biosamples": {
    "T_DND-41": {
      ...
      "tracks": [{"assay": "H2AFZ", "col": 0, "depth": 24684534, "read_length": 36,
                  ...
                  "signal_bigwig_accession": "ENCFF…", "signal_output_type": "signal p-value",
                  "kinds": ["counts", "peaks"]}]
    }
  },
...
### The pval layer's units are stamped per track, and a mixed corpus fails the build

`--signal-provenance configs/signal_provenance.eic.json` puts `signal_bigwig_accession` and its
ENCODE `signal_output_type` on every track that has a `pval` layer, and the manifest gains a
top-level `"signal": {"declared_output_type": "signal p-value", "provenance": "…"}`. A track whose
recorded `output_type` is anything else raises `manifest.py::SignalSemanticsConflict` — the build
of a corpus whose p-value layer is not all p-values now stops instead of succeeding quietly.

This exists because ENCODE ships **no** p-value track for DNase-seq: 40 of EIC's 363 signal
bigWigs are `read-depth normalized signal`, and until now the bigWig accession was recorded in no
artifact that travelled with the data (`file_metadata.json` holds only the BAM, `navigation.json`
only paths), so nothing could have caught it. `--no-strict` downgrades the conflict to a
`metadata_gaps` entry for triage, and `verify` still reports it — so it cannot be laundered.
`plan/BENCHMARK_DESIGN.md` §10 owns the fix; the provenance table is regenerated by
`tools/build_signal_provenance.py`.
