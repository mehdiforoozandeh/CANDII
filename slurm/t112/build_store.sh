...
#   KIT=/scratch/mforooz/t112_cf/code/C15 sbatch "$KIT/slurm/t112/build_store.sh"
#
...
# documented recipe (requirements-fir.txt from the CVMFS wheelhouse, then requirements-pypi.txt,
# which is x-transformers only and needs internet — compute nodes have it), from the kit snapshot.
...
module load python/3.10.13 || die "module load python" 4
...
pip install -r "$KIT/requirements-pypi.txt" || die "pip install requirements-pypi.txt" 4
