#!/bin/bash
# t118 exploratory, CPU copy of explore_x.sh (the 10 GB slice nodes were held by the scheduler for
# a higher-priority job on 2026-09-25): design X (g reads the bin value as well as C, C'), outside the pre-registered
...
#       $KIT/slurm/t118/explore_x_cpu.sh $KIT $PRODUCTS $CACHE $OUT
#SBATCH --account=def-maxwl
#SBATCH --job-name=t118L_xplcpu
#SBATCH --time=8:00:00
...
python3 "$KIT/tools/t118/ladder/train.py" train "$MANIFEST" "$COVARIATES" "$CACHE" "$RUNS" X "$G" "$SPACE" "$MODEL" "$SEED" --device cpu
python3 "$KIT/tools/t118/ladder/score.py" trained "$MANIFEST" "$COVARIATES" "$CACHE" "$BLACKLIST" "$RUNS/$RUN" --workers 8
