...
__all__ = [
    "CSV_COLUMNS",
    "MetadataConflict",
    "SignalSemanticsConflict",
    "SIGNAL_OUTPUT_TYPE",
    "AMBIGUOUS",
    "read_metadata_csvs",
    "read_file_metadata",
    "read_signal_provenance",
    "build_manifest",
    "write_manifest",
    "verify_store",
]

#: What the corpus DECLARES its `pval` layer to be. ENCODE's own name for the quantity, so a
#: track's portal `output_type` can be compared against it without a translation table. A track
#: whose recorded `output_type` is anything else is a units defect and fails the build.
SIGNAL_OUTPUT_TYPE = "signal p-value"
...
class SignalSemanticsConflict(StoreError):
    """A track's recorded bigWig `output_type` is not what the corpus declares its pval layer to be.

    ENCODE ships no p-value track for DNase-seq: 40 of EIC's 363 signal bigWigs are
    `read-depth normalized signal` where the layer claims `-log10 p`. That was invisible for as
    long as it was, because the bigWig accession travelled with no artifact — `file_metadata.json`
    records only the BAM. Stamping the accession and its `output_type` per track is what makes it
...
def read_signal_provenance(path: Path | str) -> dict:
    """`(biosample, assay) -> {"signal_bigwig_accession": …, "output_type": …}`.

    The file is generated by `tools/build_signal_provenance.py`, which joins the downloader's
    `download_plan_eic.json` (it carries `signal_bigwig_accession` for all 363 tracks) with a
    per-file sweep of the ENCODE portal for each accession's `output_type`. Shape:

        {"corpus": "eic", "declared_output_type": "signal p-value",
         "tracks": {"<biosample>": {"<assay>": {"signal_bigwig_accession": "ENCFF…",
                                                "output_type": "signal p-value", …}}}}
    """
    p = Path(path)
    if not p.is_file():
        raise FileNotFoundError(f"signal provenance not found: {p}")
    obj = json.loads(p.read_text(encoding="utf-8"))
    tracks = obj.get("tracks")
    if not isinstance(tracks, dict):
        raise StoreError(f"{p}: expected a 'tracks' object mapping biosample -> assay -> record")
    out: dict = {}
    for bios, per_assay in tracks.items():
        if not isinstance(per_assay, dict):
            raise StoreError(f"{p}: tracks[{bios!r}] is not an object")
        for assay, rec in per_assay.items():
            if not isinstance(rec, dict):
                raise StoreError(f"{p}: tracks[{bios!r}][{assay!r}] is not an object")
            out[(bios, assay)] = rec
    return out
...
def build_manifest(
    ...
    *,
    ...
    strict: bool = True,
    signal_provenance: Optional[Path | str] = None,
    signal_output_type: str = SIGNAL_OUTPUT_TYPE,
) -> dict:
    ...
    provenance = read_signal_provenance(signal_provenance) if signal_provenance else None
    gaps: list = []
    conflicts: list = []
    ...
    for name in names:
        ...
        for col, track in enumerate(tracks):
            ...
            # §10 Phase 0 — the bigWig behind the pval layer, and what ENCODE calls its units.
            # Checked only where the track HAS a pval layer: that is the layer whose units are
            ...
            if provenance is not None and "pval" in rec["kinds"]:
                ...
                if prec is None:
                    gaps.append({"biosample": name, "track": track,
                                 "field": "signal_bigwig_accession", "reason": "absent"})
                else:
                    ...
                    if rec["signal_output_type"] != signal_output_type:
                        signal_conflicts.append(
                            {"biosample": name, "track": track,
                             "accession": rec["signal_bigwig_accession"],
                             "output_type": rec["signal_output_type"],
                             "declared": signal_output_type}
                        )
            records.append(rec)
    ...
    if signal_conflicts:
        ...
        for c in signal_conflicts:
            by_type.setdefault(c["output_type"], []).append(c)
        head = "\n".join(
            f"  {c['biosample']}/{c['track']}: {c['accession']} is {c['output_type']!r}"
            for c in signal_conflicts[:20]
        )
        msg = (
            f"{len(signal_conflicts)} track(s) whose signal bigWig output_type is not the "
            f"corpus's declared {signal_output_type!r} "
            f"({ {k: len(v) for k, v in by_type.items()} }):\n{head}\n"
            f"The pval layer of those tracks is not -log10 p. See BENCHMARK_DESIGN.md §10."
        )
        if strict:
            raise SignalSemanticsConflict(msg)
        print(f"WARNING: {msg}", flush=True)
        gaps.extend(
            {"biosample": c["biosample"], "track": c["track"], "field": "signal_output_type",
             "reason": f"output_type {c['output_type']!r} != declared {signal_output_type!r}"}
            for c in signal_conflicts
        )
    ...
    manifest = {
        ...
        "signal": {
            ...
            "provenance": str(signal_provenance) if signal_provenance is not None else None,
        },
...
def verify_store(corpus_root: Path | str) -> list:
    ...
    declared = (manifest.get("signal") or {}).get("declared_output_type")

    for name, entry in manifest.get("biosamples", {}).items():
        track_names = [t["assay"] for t in entry["tracks"]]
        # §10 Phase 0 — a manifest built with --no-strict records the units defect instead of
        # raising on it. It still has to show up here, or --no-strict would launder it.
        if declared:
            for t in entry["tracks"]:
                ot = t.get("signal_output_type")
                if ot is not None and ot != declared:
                    problems.append(
                        f"{name}/{t['assay']}: signal bigWig {t.get('signal_bigwig_accession')} "
                        f"is {ot!r}, not the declared {declared!r} — its pval layer is not "
                        f"-log10 p (BENCHMARK_DESIGN.md §10)"
                    )
